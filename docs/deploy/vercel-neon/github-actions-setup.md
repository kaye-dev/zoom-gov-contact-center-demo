...
1. `main`をbranch protectionまたはrulesetで保護し、Pull Request reviewと必須CIを設定する。`Plan artifact guard / Verify plan artifacts`をrequired status checkに登録し、`plan/template.md`以外のplan生成物をmergeできないようにする。
