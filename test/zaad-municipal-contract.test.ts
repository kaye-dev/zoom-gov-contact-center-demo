...
test("RECHECK-LATEST: latest withdrawal, identity, business completion and scope prevent dispatch", () => {
  ...
  const base = { purpose: "PROCEDURE_SUPPORT" as const, contact: { status: "ACTIVE", identityVerified: true, phoneVerified: true, district: "central", preferences: [{ topic: "procedure-support", enabled: true, consentVersion: MUNICIPAL_CONSENT_VERSION, consentedAt: new Date("2026-09-01"), confirmedAt: new Date("2026-09-02") }] }, businessEvidence: { purpose: "PROCEDURE_SUPPORT", procedureRef: "procedure-1", noticeSentAt: "2026-09-01T00:00:00Z", reminderAt: "2026-09-03T00:00:00Z", status: "INCOMPLETE", statusVerifiedAt: "2026-09-08T00:00:00Z", individualCallRequired: true, reason: "職員確認済み", deadline: "2026-09-10T00:00:00Z" }, excluded: false, dueAt: new Date("2026-09-10"), now, schedule: window, dispatch: true };
  assert.equal(eligibleMunicipalTarget(base).eligible, true);
  for (const patch of [{ contact: { ...base.contact, status: "WITHDRAWN" } }, { contact: { ...base.contact, phoneVerified: false } }, { contact: { ...base.contact, preferences: [] } }, { excluded: true }, { businessEvidence: { ...base.businessEvidence, status: "VERIFIED_COMPLETE" } }, { businessEvidence: { ...base.businessEvidence, individualCallRequired: false } }]) assert.equal(eligibleMunicipalTarget({ ...base, ...patch }).eligible, false);
