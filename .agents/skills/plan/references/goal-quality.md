...
An explicit `$implement` invocation is the approval for the resolved goal, current prototype revision, and validation-profile digest. `$implement` runs `preflight --context implement`, never rebuilds the approved prototype, writes `approval.json`, and performs no Browser work before or during production editing. After static verification, the normal run executes deterministic `coverage`; `full` stays a separate release, CI, scheduled, or user-explicit run. Passing finalization writes schema-version-4 `implementation-parity.json`; a missing final file means automated coverage did not complete.
