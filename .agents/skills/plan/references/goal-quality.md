...
For a user-visible change, keep the acceptance contract in `plans/<slug>/prototype/ui-contract.json` and record exactly `approval contract: plans/<slug>/prototype/ui-contract.json — version 1`. Keep deterministic setup, target-level Browser theme setup, and probes in `plans/<slug>/prototype/parity-spec.json`; new Browser-enabled plans record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 2`. Version 1 is legacy static/custom-adapter compatibility only. Read [parity-runner.md](parity-runner.md) when authoring, implementing, or reviewing these files.
...
`parity-spec.json` version 2 contains a setup for every target/state pair, a complete `browserSetups` entry for every target, an allowlisted probe inventory, and a mapping for every manifest row. It must not contain arbitrary JavaScript, credentials, fragments, or external URLs. The common runner validates both files and selects representative smoke rows, an exact targeted selection, or a justified full matrix by phase.
...
Use `targeted` by default for copy, isolated component behavior, accessibility, keyboard/focus, and viewport-specific changes. Use `full` only for prototype/contract, global style/token, shell layout/navigation structure, cross-breakpoint responsive, multiple unrelated targets, or an explicit user/release requirement. Do not infer full scope from file count or shared-component ownership alone, and do not build a task-specific Browser adapter or runtime shim during feature implementation.

Any goal, prototype, manifest, or profile change after approval capture invalidates that `$implement` invocation and requires a new explicit invocation. Baseline-source or checkout/mount drift stops before editing. Runtime, fixture, authorization, query, route, or Browser-condition drift fails the final review. A related change after final parity invalidates final evidence and requires one replacement final run at the next completion boundary.
