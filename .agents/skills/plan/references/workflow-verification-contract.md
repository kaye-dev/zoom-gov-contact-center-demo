...
During implementation, run affected tests, applicable lint/typecheck, and diff checks under [implementation-quality.md](../../../../.claude/rules/implementation-quality.md). Full tests/build need a concrete change-related reason. Reuse results based on path/content/time; rerun affected checks after a relevant implementation edit or failure. Shipping does not independently run local tests, lint, typecheck, build, audit, DB validation or Docker CI reproduction; preserve hooks and lightweight diff/index checks and disclose missing results. Digest files and evidence schemas are not prerequisites. New prototype checks use the selected host and TSX only; no routine Next.js build, extra comparison phase, install, performance report, or approval is added. HMR and existing process/cache reuse replace repeated preparation.
...
During implementation or an explicit repair request, an observed in-scope defect is repaired and rechecked locally. A shipping-only request does not initiate independent local validation; report unresolved required-check failures and stop dependent shipment. Browser unavailability is `UI未確認`; preserve edits and allow disclosed shipping. Keep known test/functional failures distinct. Stop repeated identical tool failures and report the limitation; validation infrastructure development is a separate scope.
