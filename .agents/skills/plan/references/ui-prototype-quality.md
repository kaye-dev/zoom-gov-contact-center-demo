...
Keep the material UI contract in `plans/<slug>/prototype/ui-contract.json` and record exactly `approval contract: plans/<slug>/prototype/ui-contract.json — version 1` in the goal. Keep deterministic state setup and assertions, target-level Browser theme setup, coverage/anchor probes, risk rows, source impact, batch policy, and artifact policy in `plans/<slug>/prototype/parity-spec.json`; record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 3` for new Browser-enabled plans. Read [parity-runner.md](parity-runner.md) for the current and legacy compatibility contracts.
...
Mutable results never go in the goal, manifest, or profile. `$implement` records approval, defers Browser work until the completion candidate, and writes one schema-version-4 result under `plans/<slug>/evidence/<run-id>/implementation-parity.json`. Normal implementation uses deterministic coverage across every target's states, viewports, and themes, supplemented by risk and anchor rows. Full Cartesian parity runs separately for release, CI, scheduled, or user-explicit verification. New runs do not create pre-edit or affected Browser evidence. Any unexplained difference, missing coverage, failed required probe, or condition drift prevents automated completion.
