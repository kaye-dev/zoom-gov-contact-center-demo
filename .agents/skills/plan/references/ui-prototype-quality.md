...
Keep the material UI contract in `plans/<slug>/prototype/ui-contract.json` and record exactly `approval contract: plans/<slug>/prototype/ui-contract.json — version 1` in the goal. Keep deterministic state setup, target-level Browser theme setup, and probes in `plans/<slug>/prototype/parity-spec.json` and record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 2` for new Browser-enabled plans. Read [parity-runner.md](parity-runner.md) for the schema, version-1 compatibility boundary, and phase contract.
