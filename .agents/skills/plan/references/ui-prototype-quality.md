...
- Verify the viewports and boundary sides derived from the adopted goal and applicable source; include each material responsive risk.
...
Keep the material UI contract in `plans/<slug>/prototype/ui-contract.json` and record exactly `approval contract: plans/<slug>/prototype/ui-contract.json — version 3` in the goal. Keep deterministic state setup and assertions, target-level Browser theme setup, coverage/anchor probes, risk rows, source impact, batch policy, and artifact policy in `plans/<slug>/prototype/parity-spec.json`; record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 5` for new Browser-enabled plans. Read [parity-runner.md](parity-runner.md) for the current and legacy compatibility contracts.
...
Mutable results never go in the goal, manifest, or profile. Keep stable unchecked `UI-CHECK-XX` items for optional user verification. UI `$implement` requires CLI checks, final real-app Browser/CDP coverage, declared interactions and runtime actions, and Codex visual/requirement audit. Read [fidelity-audit.md](fidelity-audit.md). Evidence schema 6 is required for new model completion; schema 5 remains the existing matrix contract; human approval is independent and optional.
