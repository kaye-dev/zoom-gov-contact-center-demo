...
Mutable results never go in the goal, manifest, or profile. Translate the material runtime and human judgments into stable unchecked `UI-CHECK-XX` items under the goal's `## ユーザー動作確認`. `$implement` records approval and performs only static verification; it does not start Browser work, use the run lifecycle, or write `implementation-parity.json`. Deterministic coverage and full Cartesian parity remain separate release, CI, scheduled, or user-explicit verification tasks. Their absence does not prevent normal implementation completion or static review.
