...
3. Use a lowercase kebab-case slug other than `tmp` or `reviews`. Write only `plan/<slug>/goal.md` and, for UI work, `plan/<slug>/prototype/**`.
...
5. Read `plan/template.md` immediately before writing. Preserve its six H1 headings and their order, and write in Japanese unless requested otherwise.
...
2. Create the full affected screen under `plan/<slug>/prototype/` using production Tailwind utilities and `app/globals.css`; build it with `build-prototype-css.mjs`.
3. Create `ui-contract.json` version 1 and record exactly `approval contract: plan/<slug>/prototype/ui-contract.json — version 1`.
4. Create `parity-spec.json` version 1 following [references/parity-runner.md](references/parity-runner.md), then record exactly `validation profile: plan/<slug>/prototype/parity-spec.json — version 1`.
