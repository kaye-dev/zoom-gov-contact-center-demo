...
4. Create `parity-spec.json` version 4 following [references/parity-runner.md](references/parity-runner.md), then record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 4`. Declare target-level `browserSetups`, state identity assertions, coverage/anchor probe tiers, deterministic target/viewport/theme order, at least one anchor row per target, concrete risk rows, complete source impact, fixed batch policy, and artifact policy. Add the version 4 fidelity contract: stable requirement IDs, phase comparisons, requirement-derived interaction groups/strength/constraints, ordinary runtime actions, static checks, and Codex visual checks. Preserve versions 1, 2, and 3 only when revising legacy plans that will not enter the current implementation workflow.
5. Compute and record the current `prototype revision` with `prototype-revision.mjs`. Keep the complete Cartesian matrix and immutable row IDs in `ui-contract.json`; summarize its intent, full row count, covering-matrix count, anchors, and risks in the goal. For oversized manifests, preserve the complete contract and follow [Manifest size recovery policy](references/manifest-storage.md). Do not reduce acceptance coverage to fit a storage limit or describe planned storage support as implemented.
6. Run one `parity-runner.mjs preflight plans/<slug>/prototype --context plan --target <changed-target> --state <changed-state>` with repeated target/state flags for the complete affected scope and only the concrete `--viewport` / `--risk` flags needed to widen representative axes; every declared risk-row coordinate is additive automatically. Require its goal/contract/profile/source checks and deterministic full/coverage/selected counts to pass. Use that exact selected scope when, only after the goal and prototype are otherwise ready to return, you open the closest live route and prototype in the Browser for one targeted `smoke`. Ordinary smoke uses representative desktop and 390×844 in light; add only the concrete theme, breakpoint, dialog, menu, keyboard, or focus interaction at risk.
...
For oversized manifests, preserve the complete contract and follow [Manifest size recovery policy](references/parity-runner.md#manifest-size-recovery-policy). Do not reduce acceptance coverage to fit a storage limit or describe planned storage support as implemented.
