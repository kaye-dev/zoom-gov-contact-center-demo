...
4. Create `parity-spec.json` version 2 following [references/parity-runner.md](references/parity-runner.md), including complete target-level `browserSetups`, then record exactly `validation profile: plans/<slug>/prototype/parity-spec.json — version 2`. Preserve version 1 only when revising a legacy plan that will not use the in-app Browser adapter.
