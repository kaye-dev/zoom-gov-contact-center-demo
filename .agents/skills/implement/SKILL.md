...
4. For current UI work, require `ui-contract.json` version 1 and `parity-spec.json` version 3. Validate the goal's approval-contract, validation-profile, prototype-revision, and both artifacts with `prototype-revision.mjs` and `parity-runner.mjs validate`. Versions 1 and 2 remain legacy read-only inputs and are not migrated during implementation.
...
- Run focused tests plus applicable lint/typecheck and `git diff --check`. Run the full test suite only when the change can affect unrelated suites or no reliable target exists. Run a production build only for route, configuration, bundling, server-boundary changes, or an explicit repository requirement.
- Reuse passing static checks when source has not changed. After a source fix, rerun only checks affected by that source unless repository policy requires broader validation.

## Final coverage run

Only after implementation, static checks, any justified build, and diff review are complete:

1. Start or reuse the verified real app with `./dev-compose.sh ensure`, read its URL with `./dev-compose.sh status --url`, and Start the prototype once. Recheck owner, health, project, checkout mount, container ID, route, fixture, authorization, query, captured digests, and baseline resources.
2. Use the common `in-app-browser-parity-adapter.mjs` and `prepare-run` / `next-batch` / `record-batch` / `record-failure` / `invalidate-run` / `resume-run` / `finalize-run` protocol from [parity-runner.md](../plan/references/parity-runner.md). Do not create task-specific adapters, executable bundles, or runtime shims.
