...
4. For UI work, read [workflow-verification-contract.md](../plan/references/workflow-verification-contract.md) and `.claude/rules/dev-server.md`. Resolve the actual prototype contents and any later direct UI instructions. Legacy parity descriptions are historical validation data; retain product requirements and use smoke for this workflow.
...
- Implement the adopted behavior using existing components and conventions. Use focused checks during editing.
...
1. View the affected UI and refer to the prototype for its intended structure. Check major missing, overlapping, clipped, overflowing, or unusable areas.
...
Report changed paths, actual checks and smoke outcomes, known failures, unverified items, and unchecked human UI items. Keep progress in the report, not in the goal. Review and shipping reuse valid results rather than rerunning Browser for an engineering phase change.
