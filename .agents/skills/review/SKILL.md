...
- For current UI work, read [../plan/references/parity-runner.md](../plan/references/parity-runner.md), run the trusted helper `node .agents/skills/plan/scripts/prototype-revision.mjs plans/<slug>/prototype`, and validate `parity-spec.json` version 3, the selected run's `approval.json`, and schema-version-4 `implementation-parity.json` before reviewer work. Record the exact helper command in `review-data.json.validations`; on success use `status: "passed"` and include the current `sha256:` revision, and on failure use `status: "failed"` without substituting a manual hash. Recompute the exact `coverage` or `full` row set, target-state/target-viewport/target-theme coverage, risk rows, anchor rows, probe tiers, artifact digests, checkpoint history, runtime/source/profile digests, and terminal cleanup. Confirm the exact `.codex/parity-runs/<run-id>/` workspace is absent after successful finalization. A residual workspace, missing artifact, incomplete coverage, stale digest, or failed cleanup is a major finding. New runs do not create `pre-edit-parity.json`.
