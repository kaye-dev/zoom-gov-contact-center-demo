...
- For UI work, use validation entries for the static goal/prototype preflight, approval digest, checklist contract, and checks actually run. A missing `implementation-parity.json`, Browser run, screenshot, cleanup result, human approval, or full-parity result is normal and must not be recorded as a failed validation.
- When an existing parity run is explicitly in review scope, use separate validation entries for automated state/viewport/theme coverage, risk rows, anchor rows, cleanup, human visual approval, and full parity. Never collapse `automationCoverageStatus`, `humanVisualApprovalStatus`, and `fullParityStatus` into one pass/fail statement. Evidence lists may then reference representative screenshot/URL and compact artifact records; do not copy raw screenshot bytes, raw DOM, accessibility trees, workspace fragments, or large runner JSON into `review-data.json`.
