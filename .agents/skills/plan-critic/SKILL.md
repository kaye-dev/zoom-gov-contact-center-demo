...
- Start a fresh no-history `independent_reviewer` custom agent with only the authoritative requirements bundle, exact goal/prototype, applicable rules, and necessary repository/runtime evidence. Do not pass a model or reasoning override. Ask it for atomic requirement closure, factual, interface, safety, verification, and UI-parity findings, including exactly five Markdown columns in every closure row; it must not edit files. Stop if the custom agent or its configured model is unavailable rather than substituting another reviewer.
