...
Shipping uses valid focused-check and UI-smoke results; it does not run Browser or detailed parity. Browser unavailability is disclosed as UI unverified. A known functional/test failure is not merely unverified and must be reported as a blocker unless the user explicitly accepts that exact failure for shipment.
