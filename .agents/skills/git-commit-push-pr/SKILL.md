...
- Reuse valid implementation results. Do not independently run local test/lint/typecheck/build/audit, DB validation or Docker CI reproduction for commit/push/PR operations, even when results are missing; disclose unverified checks. Preserve hooks and the lightweight diff/index checks below. Separately requested implementation or validation follows [implementation-quality.md](../../../.claude/rules/implementation-quality.md).
- When an error occurs, diagnose it and fix directly related minor issues within the task scope without another prompt, preserving intended behavior, permissions, data contracts and test strength. Do not automatically add independent local validation during shipping. Stage only explicit repair paths; if required-check failures remain, or repair requires validation, major changes or unresolved decisions, report the cause and proposed response and stop dependent shipping operations. Missing results alone are not known failures.
