...
- Use an existing PR's base; do not change it automatically. For a new PR, use the base branch from which the task started, including `main` for ordinary changes started there. Determine it from the user's instruction, task-start branch, or recorded branch/worktree creation context, and verify the base-relative commits and diff match the task. Do not assume `develop` or infer the base solely from a topic's upstream tracking ref. Ask only when the starting base cannot be identified or explicit choices conflict.
