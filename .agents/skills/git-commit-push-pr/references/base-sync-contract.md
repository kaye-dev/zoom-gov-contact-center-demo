...
Read this reference immediately before synchronizing a topic branch with its remote base. Use it only for explicitly requested base synchronization or the optional `$plan-finalize` operation. Ordinary shipping can push a topic whose base has advanced.
