...
For user-visible work, treat the approved prototype and its `UI契約` parity matrix as the implementation target. Replace simulated data and side effects with production behavior without silently changing screen structure, copy, component choice, layout, responsive behavior, interactions, or states. If a material deviation is necessary, stop and obtain an updated UI contract and user approval before continuing. Before checking G05, compare the live implementation with every approved prototype route, overlay, viewport, and state in the same matrix; record and fix unexplained differences.
