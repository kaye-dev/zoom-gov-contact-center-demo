...
- light/dark、390px、関連breakpointの直前／直後、長い翻訳、readonly・saving・error・success、keyboard／focusを設計する。UI変更の`$implement`では静的検証後にCodexアプリ内Browserで大きなUI崩れと主要正常系を通常1〜3代表シナリオで確認し、未チェックのユーザー動作確認を別に引き渡す。状態の設計とBrowserでの総当たり検証を混同しない。
