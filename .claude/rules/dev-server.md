...
- prototypeは本番と同じTailwind utilityと`app/globals.css`を使う。plan中は変更target/stateを代表desktopと390×844でtargeted smoke確認し、具体的なtheme・responsive・interaction riskだけを追加する。`$implement`中はcoverageを既定とし、fullは許可された独立実行に限定する。
