...
- 新規UI`$implement`は承認境界で`plans/<slug>/evidence/<run-id>/approval.json`を作り、final coverageとcleanupがpassした場合だけ同runへschema-version-4 `implementation-parity.json`を作成する。最終fileはcoverage/full scope、exact row、全軸coverage、risk、anchor、checkpoint、artifact、cleanupと自動coverage・人間承認・fullの独立statusを持つ。既存schema version 1から3はread-only互換として扱い、task固有adapterやruntime shimをfeature実装中に作らない。
