...
export function AdminPageTitleHelp({ title, description, label }: { title: string; description: string; label: string }) {
  const id = useId();
  ...
  return (
    <div ref={group} className="relative flex w-fit max-w-full items-center gap-2">
      ...
      <div
        ...
        onPointerEnter={(event) => enter(event.pointerType)}
        onPointerLeave={leave}
        className={open ? "absolute left-0 top-full z-50 mt-2 w-80 max-w-[calc(100vw-2.5rem)] rounded-lg bg-fg px-3 py-2 text-sm leading-6 text-surface shadow-lg" : "sr-only"}
      >
        {description}
      </div>
    </div>
