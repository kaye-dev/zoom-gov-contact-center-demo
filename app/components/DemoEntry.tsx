...
export function DemoFrame({ copy, children }: { copy: DemoEntryCopy; children: ReactNode }) {
  return <div className="min-h-screen flex flex-col bg-surface text-fg"><header className="sticky top-0 z-50 border-t-4 border-t-accent border-b border-b-line bg-surface"><div className="mx-auto max-w-6xl px-5 py-6 flex items-center justify-center"><span className="font-bold text-lg">{copy.brand}</span></div></header>{children}<footer className="mt-auto border-t border-line"><p className="mx-auto max-w-6xl px-5 py-6 text-center text-sm text-fg-muted">{copy.footer}</p></footer></div>;
}
