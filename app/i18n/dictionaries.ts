...
const reservationApiKeyCopy: Record<Locale, ReservationApiKeyDictionary> = {
  ja: {
    ...
    api: { title: '公開API', description: 'Authorization ヘッダーに Bearer 形式でAPIキーを指定します。各権限は個別に付与できます。', method: 'メソッド', endpoint: 'エンドポイント', permission: '権限', operation: '操作', descriptions: { LIST: '予約一覧の取得', READ: '指定した予約の取得', CREATE: '予約の登録', UPDATE: '予約の変更', DELETE: '予約の削除' } },
    ...
    logs: {
      ...
      list: { title: '実行履歴', description: '新しい実行順に50件ずつ表示します。', count: '{count}件', requestedAt: '実行日時', apiKey: '実行APIキー', method: 'メソッド', api: 'API', result: '結果', duration: '処理時間', requestId: 'リクエストID', next: '次の50件', emptyTitle: '条件に一致する利用ログはありません', emptyDescription: '検索条件を変更するか、公開APIの実行後にもう一度確認してください。' },
      detail: { back: 'API利用ログに戻る', title: 'API利用ログの詳細', request: 'リクエスト', normalizedJson: '正規化済みJSON', credentialNotice: '認証ヘッダーと未解析のリクエスト本文は保存しません。', response: 'レスポンス', httpStatus: 'HTTP {status}', success: '成功', failure: '失敗', json: 'JSON', properties: 'プロパティ', requestedAt: '実行日時', apiKey: 'APIキー', method: 'メソッド', permission: '権限', status: 'ステータス', duration: '処理時間', completedAt: '完了日時', requestId: 'リクエストID', created: 'Created', requestCopyLabel: 'リクエストJSONをコピー', requestCopied: 'リクエストJSONをコピーしました。', requestCopyFailed: 'リクエストJSONをコピーできませんでした。', responseCopyLabel: 'レスポンスJSONをコピー', responseCopied: 'レスポンスJSONをコピーしました。', responseCopyFailed: 'レスポンスJSONをコピーできませんでした。' },
    },
  ...
  en: {
    ...
    api: { title: 'Public API', description: 'Send the API key as a Bearer token in the Authorization header. Permissions are granted independently.', method: 'Method', endpoint: 'Endpoint', permission: 'Permission', operation: 'Operation', descriptions: { LIST: 'List reservations', READ: 'Get the specified reservation', CREATE: 'Create a reservation', UPDATE: 'Update a reservation', DELETE: 'Delete a reservation' } },
  ...
  'zh-Hans': {
    ...
    logs: {
      ...
      list: { title: '执行记录', description: '按最新执行顺序每次显示50条。', count: '{count}条', requestedAt: '执行时间', apiKey: '执行API密钥', method: '方法', api: 'API', result: '结果', duration: '处理时间', requestId: '请求ID', next: '后50条', emptyTitle: '没有符合条件的使用日志', emptyDescription: '请更改筛选条件，或在公开API执行后再次查看。' },
      detail: { back: '返回API使用日志', title: 'API使用日志详情', request: '请求', normalizedJson: '规范化JSON', credentialNotice: '不保存认证标头和未经解析的请求正文。', response: '响应', httpStatus: 'HTTP {status}', success: '成功', failure: '失败', json: 'JSON', properties: '属性', requestedAt: '执行时间', apiKey: 'API密钥', method: '方法', permission: '权限', status: '状态', duration: '处理时间', completedAt: '完成时间', requestId: '请求ID', created: '已创建', requestCopyLabel: '复制请求JSON', requestCopied: '已复制请求JSON。', requestCopyFailed: '无法复制请求JSON。', responseCopyLabel: '复制响应JSON', responseCopied: '已复制响应JSON。', responseCopyFailed: '无法复制响应JSON。' },
    },
  ...
  'zh-Hant': {
    ...
    logs: {
      ...
      list: { title: '執行紀錄', description: '依最新執行順序每次顯示50筆。', count: '{count}筆', requestedAt: '執行時間', apiKey: '執行API金鑰', method: '方法', api: 'API', result: '結果', duration: '處理時間', requestId: '請求ID', next: '後50筆', emptyTitle: '沒有符合條件的使用紀錄', emptyDescription: '請變更篩選條件，或在公開API執行後再次查看。' },
      detail: { back: '返回API使用紀錄', title: 'API使用紀錄詳細資料', request: '請求', normalizedJson: '正規化JSON', credentialNotice: '不會儲存驗證標頭及未解析的請求本文。', response: '回應', httpStatus: 'HTTP {status}', success: '成功', failure: '失敗', json: 'JSON', properties: '屬性', requestedAt: '執行時間', apiKey: 'API金鑰', method: '方法', permission: '權限', status: '狀態', duration: '處理時間', completedAt: '完成時間', requestId: '請求ID', created: '已建立', requestCopyLabel: '複製請求JSON', requestCopied: '已複製請求JSON。', requestCopyFailed: '無法複製請求JSON。', responseCopyLabel: '複製回應JSON', responseCopied: '已複製回應JSON。', responseCopyFailed: '無法複製回應JSON。' },
    },
  ...
  ko: {
    ...
    api: { title: '공개 API', description: 'Authorization 헤더에 Bearer 형식으로 API 키를 지정합니다. 각 권한은 독립적으로 부여됩니다.', method: '메서드', endpoint: '엔드포인트', permission: '권한', operation: '작업', descriptions: { LIST: '예약 목록 조회', READ: '지정한 예약 조회', CREATE: '예약 등록', UPDATE: '예약 변경', DELETE: '예약 삭제' } },
    ...
    logs: {
      ...
      list: { title: '실행 기록', description: '최신 실행 순으로 50건씩 표시합니다.', count: '{count}건', requestedAt: '실행 시간', apiKey: '실행 API 키', method: '메서드', api: 'API', result: '결과', duration: '처리 시간', requestId: '요청 ID', next: '다음 50건', emptyTitle: '조건에 맞는 사용 로그가 없습니다', emptyDescription: '검색 조건을 변경하거나 공개 API 실행 후 다시 확인하세요.' },
      detail: { back: 'API 사용 로그로 돌아가기', title: 'API 사용 로그 상세', request: '요청', normalizedJson: '정규화된 JSON', credentialNotice: '인증 헤더와 파싱되지 않은 요청 본문은 저장하지 않습니다.', response: '응답', httpStatus: 'HTTP {status}', success: '성공', failure: '실패', json: 'JSON', properties: '속성', requestedAt: '실행 시간', apiKey: 'API 키', method: '메서드', permission: '권한', status: '상태', duration: '처리 시간', completedAt: '완료 시간', requestId: '요청 ID', created: '생성됨', requestCopyLabel: '요청 JSON 복사', requestCopied: '요청 JSON을 복사했습니다.', requestCopyFailed: '요청 JSON을 복사하지 못했습니다.', responseCopyLabel: '응답 JSON 복사', responseCopied: '응답 JSON을 복사했습니다.', responseCopyFailed: '응답 JSON을 복사하지 못했습니다.' },
    },
