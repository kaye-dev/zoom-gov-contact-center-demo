...
export type ReservationApiKeyDictionary = {
  ...
  keys: { title: string; description: string; name: string; key: string; nameKey: string; permissions: string; monthlyLimit: string; monthlyUsage: string; status: string; created: string; lastUsed: string; actions: string; active: string; revoked: string; never: string; unlimited: string; remaining: string; changeLimit: string; revoke: string; emptyTitle: string; emptyDescription: string };
...
const reservationApiKeyCopy: Record<Locale, ReservationApiKeyDictionary> = {
  ja: {
    ...
    keys: { title: '発行済みAPIキー', description: 'キー本体は発行直後に一度だけ表示されます。APIキーごとに月間上限と利用状況を管理できます。', name: '名前', key: 'APIキー', nameKey: '名前 / キー', permissions: '権限', monthlyLimit: 'キーごとの月間上限', monthlyUsage: '今月の利用', status: '状態', created: '発行日時', lastUsed: '最終利用', actions: '操作', active: '有効', revoked: '無効', never: '未利用', unlimited: '上限なし', remaining: '残り', changeLimit: '上限を変更', revoke: '無効化', emptyTitle: 'APIキーはまだ発行されていません', emptyDescription: '外部システムごとに必要な権限だけを選んで発行してください。' },
  ...
  'zh-Hans': {
    ...
    keys: { title: '已签发API密钥', description: '密钥本身仅在签发后显示一次。可在此管理各API密钥的月度上限和使用量。', name: '名称', key: 'API密钥', nameKey: '名称 / 密钥', permissions: '权限', monthlyLimit: '单个密钥每月上限', monthlyUsage: '本月使用量', status: '状态', created: '签发时间', lastUsed: '最后使用', actions: '操作', active: '有效', revoked: '已撤销', never: '未使用', unlimited: '无限制', remaining: '剩余', changeLimit: '修改上限', revoke: '撤销', emptyTitle: '尚未签发API密钥', emptyDescription: '请为每个外部系统仅选择必要权限后签发。' },
  ...
  'zh-Hant': {
    ...
    keys: { title: '已簽發API金鑰', description: '金鑰本身只會在簽發後顯示一次。可在此管理各API金鑰的每月上限與使用量。', name: '名稱', key: 'API金鑰', nameKey: '名稱 / 金鑰', permissions: '權限', monthlyLimit: '單一金鑰每月上限', monthlyUsage: '本月使用量', status: '狀態', created: '簽發時間', lastUsed: '最後使用', actions: '操作', active: '有效', revoked: '已撤銷', never: '未使用', unlimited: '無限制', remaining: '剩餘', changeLimit: '修改上限', revoke: '撤銷', emptyTitle: '尚未簽發API金鑰', emptyDescription: '請為每個外部系統僅選擇必要權限後簽發。' },
  ...
  ko: {
    ...
    keys: { title: '발급된 API 키', description: '키 본문은 발급 직후 한 번만 표시됩니다. 각 API 키의 월간 한도와 사용량을 관리할 수 있습니다.', name: '이름', key: 'API 키', nameKey: '이름 / 키', permissions: '권한', monthlyLimit: '키별 월간 한도', monthlyUsage: '이번 달 사용', status: '상태', created: '발급 일시', lastUsed: '마지막 사용', actions: '작업', active: '활성', revoked: '폐기됨', never: '미사용', unlimited: '한도 없음', remaining: '남음', changeLimit: '한도 변경', revoke: '폐기', emptyTitle: '아직 발급된 API 키가 없습니다', emptyDescription: '외부 시스템별로 필요한 권한만 선택해 발급하세요.' },
