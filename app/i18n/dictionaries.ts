...
const reservationApiKeyCopy: Record<Locale, ReservationApiKeyDictionary> = {
  ja: {
    ...
    api: { title: '公開API', description: 'Authorization ヘッダーに Bearer 形式でAPIキーを指定します。作成・個別取得・更新・削除では、X-Reservation-Caller-Phone にE.164形式の発信者ANIを指定し、利用者が発話した番号で代用しないでください。作成時は Idempotency-Key、更新・削除時は If-Match も必要です。', method: 'メソッド', endpoint: 'エンドポイント', permission: '権限', operation: '操作', descriptions: { LIST: '予約一覧の取得', READ: '指定した予約の取得', CREATE: '予約の登録', UPDATE: '予約の変更', DELETE: '予約の削除' }, operations: { services: 'サービス一覧の取得', availability: '空き枠の取得', list: '予約一覧の取得', read: '指定した予約の取得', create: '予約の登録', replace: '予約の置換', update: '予約の変更', delete: '予約の削除' } },
  ...
  'zh-Hans': {
    ...
    api: { title: '公开API', description: '请在Authorization标头中以Bearer形式指定API密钥。创建、单项查询、更新和删除时，请在 X-Reservation-Caller-Phone 中传递E.164格式的主叫ANI，不得用用户口述的号码代替。创建时还需指定 Idempotency-Key，更新和删除时还需指定 If-Match。', method: '方法', endpoint: '端点', permission: '权限', operation: '操作', descriptions: { LIST: '获取预约列表', READ: '获取指定预约', CREATE: '创建预约', UPDATE: '修改预约', DELETE: '删除预约' }, operations: { services: '获取服务列表', availability: '获取可用时段', list: '获取预约列表', read: '获取指定预约', create: '创建预约', replace: '替换预约', update: '修改预约', delete: '删除预约' } },
  ...
  'zh-Hant': {
    ...
    api: { title: '公開API', description: '請在Authorization標頭中以Bearer形式指定API金鑰。建立、單筆查詢、更新及刪除時，請在 X-Reservation-Caller-Phone 傳送E.164格式的來電ANI，不得以使用者口述的號碼代替。建立時另需指定 Idempotency-Key，更新及刪除時另需指定 If-Match。', method: '方法', endpoint: '端點', permission: '權限', operation: '操作', descriptions: { LIST: '取得預約清單', READ: '取得指定預約', CREATE: '新增預約', UPDATE: '修改預約', DELETE: '刪除預約' }, operations: { services: '取得服務清單', availability: '取得可用時段', list: '取得預約清單', read: '取得指定預約', create: '新增預約', replace: '取代預約', update: '修改預約', delete: '刪除預約' } },
    ...
    logs: {
      ...
      detail: { back: '返回API使用紀錄', title: 'API使用紀錄詳細資料', request: '請求', normalizedJson: '正規化JSON', credentialNotice: '不會儲存驗證標頭、來電電話號碼標頭、Idempotency-Key 及未解析的請求本文。', response: '回應', httpStatus: 'HTTP {status}', success: '成功', failure: '失敗', json: 'JSON', properties: '屬性', requestedAt: '執行時間', apiKey: 'API金鑰', method: '方法', permission: '權限', status: '狀態', duration: '處理時間', completedAt: '完成時間', requestId: '請求ID', idempotencyOutcome: '冪等結果', responseLocation: 'Location', responseEtag: 'ETag', notApplicable: '—', created: '已建立', requestCopyLabel: '複製請求JSON', requestCopied: '已複製請求JSON。', requestCopyFailed: '無法複製請求JSON。', responseCopyLabel: '複製回應JSON', responseCopied: '已複製回應JSON。', responseCopyFailed: '無法複製回應JSON。' },
    },
  ...
  ko: {
    ...
    api: { title: '공개 API', description: 'Authorization 헤더에 Bearer 형식으로 API 키를 지정합니다. 생성, 개별 조회, 변경 및 삭제 시 X-Reservation-Caller-Phone에 E.164 형식의 발신자 ANI를 전달하며, 사용자가 말한 번호로 대체하지 마세요. 생성 시 Idempotency-Key, 변경 및 삭제 시 If-Match도 필요합니다.', method: '메서드', endpoint: '엔드포인트', permission: '권한', operation: '작업', descriptions: { LIST: '예약 목록 조회', READ: '지정한 예약 조회', CREATE: '예약 등록', UPDATE: '예약 변경', DELETE: '예약 삭제' }, operations: { services: '서비스 목록 조회', availability: '예약 가능 시간 조회', list: '예약 목록 조회', read: '지정한 예약 조회', create: '예약 등록', replace: '예약 교체', update: '예약 변경', delete: '예약 삭제' } },
    ...
    logs: {
      ...
      detail: { back: 'API 사용 로그로 돌아가기', title: 'API 사용 로그 상세', request: '요청', normalizedJson: '정규화된 JSON', credentialNotice: '인증 헤더, 발신자 전화번호 헤더, Idempotency-Key 및 파싱되지 않은 요청 본문은 저장하지 않습니다.', response: '응답', httpStatus: 'HTTP {status}', success: '성공', failure: '실패', json: 'JSON', properties: '속성', requestedAt: '실행 시간', apiKey: 'API 키', method: '메서드', permission: '권한', status: '상태', duration: '처리 시간', completedAt: '완료 시간', requestId: '요청 ID', idempotencyOutcome: '멱등성 결과', responseLocation: 'Location', responseEtag: 'ETag', notApplicable: '—', created: '생성됨', requestCopyLabel: '요청 JSON 복사', requestCopied: '요청 JSON을 복사했습니다.', requestCopyFailed: '요청 JSON을 복사하지 못했습니다.', responseCopyLabel: '응답 JSON 복사', responseCopied: '응답 JSON을 복사했습니다.', responseCopyFailed: '응답 JSON을 복사하지 못했습니다.' },
    },
