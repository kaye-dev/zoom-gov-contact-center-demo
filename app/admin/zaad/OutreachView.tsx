"use client";
import { OutreachFeedbackProvider } from "./OutreachFeedbackProvider";
import { Feedback, InlineFeedback } from "@/app/components/admin/Feedback";
...
export function OutreachView({ tenant, allowedTenants, permissions, canConfigure, years, serverDate }: Omit<OutreachPanelProps, "setDirty"> & { allowedTenants: readonly TenantKey[]; canConfigure: boolean }) {
  ...
  return <OutreachFeedbackProvider key={`${tenant}:${selected}`} tenant={tenant} view={selected}><OutreachActionHost.Provider value={showingGroupDetail ? null : actionHost}><section>
...
export function OutreachFailure({ retry, inline = false }: { retry: () => void; inline?: boolean }) { const { t } = useI18n(); const Component = inline ? InlineFeedback : Feedback; return <Component tone="error" action={<button className={secondary} onClick={retry}>{t.admin.zaad.common.retry}</button>}>{t.admin.zaad.common.failure}</Component>; }
