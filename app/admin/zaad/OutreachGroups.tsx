...
export function OutreachGroups(props: OutreachPanelProps) {
  ...
  function edit(row?: Group) { router.push(groupHref(row?.kind === "DEFAULT" ? "default-group-bind" : row ? "group-edit" : "group-create", row?.id), { scroll: false }); }
  ...
  if ((state === "default-group-detail" || state === "default-group-bind") && id) return <OutreachDefaultGroup key={`${tenant}:${id}`} {...props} id={id} startEditing={state === "default-group-bind"} close={back} />;
