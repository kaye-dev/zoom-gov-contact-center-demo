"use client";
import { Feedback, type FeedbackTone } from "@/app/components/admin/Feedback";
import { useOutreachFeedback } from "./OutreachFeedbackProvider";
import { syncFeedbackTone, syncFeedbackMessage } from "./outreach-feedback";
...
export function OutreachGroups(props: OutreachPanelProps) {
  ...
  const [successTone, setSuccessTone] = useState<FeedbackTone>("success");
  ...
  const refresh = () => { setData(null); setFailed(false); setLoadedKey(""); setReload(value => value + 1); };
  ...
  async function synced(ids: string[]) {
    ...
    setData(result); setFailed(false); setSuccessTone("success"); setSuccess(d.groupSync.success.replace("{count}", String(ids.length))); closeSync();
  }
  const finish = () => { setDeleting(null); };
  const close = () => { if (!lock.current) finish(); };
  function open(row: Group) { setSuccess(""); router.push(groupHref(row.kind === "DEFAULT" ? "default-group-detail" : "group-detail", row.id), { scroll: false }); }
  useEffect(() => {
    if (!routed || !id) return;
  ...
  async function syncMembers(memberId?: string) {
    ...
    try {
      ...
      setSuccessTone(syncFeedbackTone(result));
      setSuccess(syncFeedbackMessage(d.feedback.syncCounts, result.counts));
  ...
  function memberActions(member: Member): TableRowAction[] {
    ...
    return [...(permissions.update && member.mapping?.personId && member.mapping.id && member.syncStatus !== "SYNCED" ? [{ id: "sync", label: d.sync, disabled: busy || Boolean(mutationBlock), onSelect: () => void syncMembers(member.mapping!.id) }] : []), ...(permissions.update ? [{ id: "edit", label: d.memberEdit, disabled: busy || Boolean(mutationBlock), disabledReason: mutationReason, onSelect: () => setMemberAction({ mode: "edit", member }) }, ...(!member.mapping?.personId || ["DIFFERENCE", "UNKNOWN", "SYNCING"].includes(member.mapping.syncState) ? [{ id: "link", label: d.link, disabled: busy, onSelect: () => setMemberAction({ mode: "link" as const, member }) }] : [])] : []), ...(permissions.create && !member.mapping?.personId ? [{ id: "import", label: d.zoomImportUi.singleTitle, disabled: busy, onSelect: () => setImporting({ groupId: detail!.group.id, contactId: member.id }) }] : []), ...(permissions.delete ? [{ id: "delete", label: d.memberDelete, tone: "danger" as const, disabled: busy || Boolean(mutationBlock), disabledReason: mutationReason, onSelect: () => setMemberAction({ mode: "delete", member }) }] : [])];
  }
  ...
  if (routed && (loadedKey !== routeKey || detailFailure === routeKey)) return <div className="space-y-4"><h1 className="text-2xl font-bold">{state === "group-edit" ? d.groupEdit : d.tabLabels["contact-lists"]}</h1><DetailPageBreadcrumb title={state === "group-edit" ? d.groupEdit : d.tabLabels["contact-lists"]} />{detailFailure === routeKey ? <OutreachFailure retry={() => { setDetailFailure(""); refresh(); }} /> : <OutreachLoading />}</div>;
  ...
  return <div>
    {!showingDetail && <><OutreachListActions><button className={outreachTabAction} disabled={busy} onClick={() => { const params = new URLSearchParams(query.toString()); params.set("section", "contacts"); router.push(`/admin/zaad?${params}`); }}>{d.defaultGroups.manageContacts}</button><button ref={syncButton} className={outreachTabAction} disabled={busy || !fullAccess || !permissions.update} title={!fullAccess || !permissions.update ? d.groupSync.permission : undefined} onClick={() => { setSuccess(""); router.push(groupHref("group-sync"), { scroll: false }); }}>{d.sync}</button>{permissions.create && <button className={outreachTabAction} disabled={busy} onClick={() => edit()}>{z.common.create}</button>}</OutreachListActions>
    {(success || (error && !deleting) || failed) && <div data-inline-feedback className="space-y-3 py-5">{success && <Feedback tone={successTone} closeLabel={d.feedback.dismiss} onClose={successTone === "success" ? () => setSuccess("") : undefined}>{success}</Feedback>}{error && !deleting && <Feedback tone="error">{error}</Feedback>}{failed && <OutreachFailure retry={refresh} />}</div>}<div className={outreachTableFrame}><table aria-busy={!data && !failed} className="w-full min-w-[760px] text-left text-sm"><thead className="bg-surface-hover"><tr><th className="px-4 py-3">{d.groupListName}</th><th className="px-4 py-3">{d.defaultGroups.listId}</th><th className="whitespace-nowrap px-4 py-3">{d.groupMembers}</th><th className="px-4 py-3 text-center">{z.residents.actions}</th></tr></thead><tbody className="divide-y divide-line [&>tr:first-child]:border-t [&>tr:first-child]:border-line">{!data && !failed ? <OutreachTableSkeleton columns={4} /> : (failed ? [] : data?.items ?? []).map(row => { const name = row.kind === "DEFAULT" ? nameOf(tenant, row.topicKey!) : row.name; return <tr key={row.id} className={busy ? "" : "cursor-pointer hover:bg-surface-hover/40"} onClick={event => handleOutreachRowClick(event, () => open(row), busy)}><td className="px-4 py-4"><button className="cursor-pointer text-left font-semibold hover:text-accent" disabled={busy} onClick={() => open(row)}>{name}</button>{row.kind === "DEFAULT" && <p className="mt-1 text-xs text-fg-muted">{d.defaultGroups.defaultLabel}</p>}</td><td className="px-4 py-3">{row.kind === "DEFAULT" ? row.contactListId ?? d.defaultGroups.missing : row.id}</td><td className="whitespace-nowrap px-4 py-3">{row.contactCount ?? "—"}</td><td className="px-4 py-3"><div className="flex justify-center">{row.kind === "DEFAULT" ? fullAccess && permissions.update && <button className="cursor-pointer whitespace-nowrap text-accent" disabled={busy} onClick={() => edit(row)}>{row.contactListId ? d.defaultGroups.update : d.defaultGroups.configure}</button> : <TableRowActions items={groupActions(row)} label={`${name}: ${z.residents.actions}`} open={menu === row.id} onOpenChange={value => setMenu(value ? row.id : null)} />}</div></td></tr>; })}</tbody></table></div>

    </>}
    {showingDetail && detail && <section className="space-y-6" aria-labelledby="group-detail-title"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2"><h1 id="group-detail-title" className="text-2xl font-bold">{detail.group.name}</h1><p className="break-all text-sm text-fg-muted">{d.defaultGroups.listId}：{detail.group.id}</p></div></div><DetailPageBreadcrumb title={detail.group.name} disabled={busy} />{permissions.create && <div className="flex flex-wrap gap-3"><button className={secondary} disabled={busy || Boolean(mutationBlock)} aria-describedby={mutationBlock ? "group-mutation-block" : undefined} onClick={() => setMemberAction({ mode: "add" })}>{d.memberAdd}</button><button className={secondary} disabled={busy} onClick={() => setImporting({ groupId: detail.group.id })}>{d.defaultGroups.importFromZoom}</button></div>}
      <div className="flex flex-wrap items-center justify-between gap-3"><p>{d.defaultGroups.unsynced.replace("{count}", String(detail.summary?.unsynced ?? "—"))} / {d.defaultGroups.total.replace("{count}", String(detail.summary?.total ?? "—"))}</p>{permissions.update && <button className={primary} disabled={busy || Boolean(mutationBlock) || detail.summary?.unsynced === 0} onClick={() => void syncMembers()}>{t.outreachCommon.defaultGroups.bulk}</button>}</div>
      {detail.providerState !== "READY" && <Feedback tone="error">{d.defaultGroups.providerFailure}</Feedback>}{success && <Feedback tone={successTone}>{success}</Feedback>}{error && !deleting && <Feedback tone="error">{error}</Feedback>}
      {mutationReason && <div className="space-y-3"><Feedback tone="warning" id="group-mutation-block">{mutationReason}</Feedback><button className={secondary} disabled={busy} onClick={refresh}>{d.reload}</button></div>}
      {!detail.items.length ? <p>{z.common.empty}</p> : <div className={outreachTableFrame}><table aria-busy={!data && !failed} className="w-full min-w-[760px] text-left text-sm"><thead className="bg-surface-hover"><tr><th className="px-4 py-3">{z.residents.name}</th><th className="px-4 py-3">{z.residents.phone}</th><th className="px-4 py-3">{d.defaultGroups.source}</th><th className="px-4 py-3">{z.messages.status}</th><th className="px-4 py-3 text-center">{z.residents.actions}</th></tr></thead><tbody className="divide-y divide-line [&>tr:first-child]:border-t [&>tr:first-child]:border-line">{detail.items.map(member => <tr key={member.id}><td className="px-4 py-3">{member.displayName}</td><td className="whitespace-nowrap px-4 py-3">{member.phones.map(phone => phone.number).join(" / ") || d.unknown}</td><td className="px-4 py-3">{member.source === "SITE" ? d.defaultGroups.site : "Zoom"}</td><td className="px-4 py-3">{d.defaultGroups.statuses[member.syncStatus ?? "REGISTERED"]}</td><td className="px-4 py-3"><div className="flex justify-center"><TableRowActions items={memberActions(member)} label={`${member.displayName}: ${z.residents.actions}`} open={menu === member.id} onOpenChange={value => setMenu(value ? member.id : null)} /></div></td></tr>)}</tbody></table></div>}
    <p className="text-sm leading-7 text-fg-muted">{d.memberBoundary}</p>
    </section>}
    {bindingGroup && fullAccess && permissions.update && <DefaultGroupBinding key={`${tenant}:${bindingGroup.id}`} tenant={tenant} group={{ ...bindingGroup, contactListId: bindingGroup.contactListId ?? null }} name={nameOf(tenant, bindingGroup.topicKey!)} setDirty={setDirty} setSaving={setSaving} close={closeBinding} saved={() => { closeBinding(); refresh(); notify({id:crypto.randomUUID(),messageKey:"defaultGroups.candidateSaved"}); }} />}
