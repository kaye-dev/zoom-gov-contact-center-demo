...
import { formatGroupUpdatedAt } from "@/lib/zaad/group-sync";
...
export function OutreachGroups(props: OutreachPanelProps) {
  ...
  const { t, locale } = useI18n(), z = t.admin.zaad, d = t.outreachCommon, router = useRouter(), query = useSearchParams();
  ...
  return <div className="space-y-5">
    {!showingDetail && <><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-bold">{d.tabs[1]} <span className="text-sm font-normal text-fg-muted">({data?.total ?? "—"})</span></h2><div className="flex flex-wrap gap-3"><button ref={syncButton} className={secondary} disabled={busy || !fullAccess || !permissions.update} title={!fullAccess || !permissions.update ? d.groupSync.permission : undefined} onClick={() => { setSuccess(""); router.push(groupHref("group-sync"), { scroll: false }); }}>{d.sync}</button>{permissions.create && <button className={primary} disabled={busy} onClick={() => edit()}>{z.common.create}</button>}</div></div>
    {success && <p role="status">{success}</p>}
    {error && !deleting && <p role="alert">{error}</p>}
    {!data?.items.length ? <p className="p-8 text-center">{z.common.empty}</p> : <div className="overflow-x-auto rounded-lg border border-line"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-line bg-surface-hover"><tr><th className="px-4 py-3">{d.groupListName}</th><th className="px-4 py-3">{z.contactLists.descriptionLabel}</th><th className="whitespace-nowrap px-4 py-3">{d.groupSync.updatedAt}</th><th className="whitespace-nowrap px-4 py-3">{d.groupMembers}</th><th className="whitespace-nowrap px-4 py-3">{d.groupIndustry}</th><th className="px-4 py-3 text-center">{z.residents.actions}</th></tr></thead><tbody className="divide-y divide-line">{data.items.map(row => <tr key={row.id}><td className="px-4 py-3"><button className="cursor-pointer text-left font-semibold hover:text-accent" disabled={busy} onClick={() => void open(row)}>{row.name}</button></td><td className="px-4 py-3">{row.description}</td><td className="whitespace-nowrap px-4 py-3">{formatGroupUpdatedAt(row.updatedAt, locale)}</td><td className="whitespace-nowrap px-4 py-3">{row.contactCount ?? "—"}</td><td className="whitespace-nowrap px-4 py-3">{t.admin.industrySettings.names[tenant]}</td><td className="px-4 py-3"><div className="flex justify-center"><TableRowActions items={groupActions(row)} label={`${row.name}: ${z.residents.actions}`} open={menu === row.id} onOpenChange={value => setMenu(value ? row.id : null)} /></div></td></tr>)}</tbody></table></div>}
    </>}
