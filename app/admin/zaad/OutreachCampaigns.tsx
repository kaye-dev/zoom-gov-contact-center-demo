...
import { MUNICIPAL_PURPOSES, purposePath, purposeTopic } from "@/lib/zaad/municipal/contracts";
...
function OutreachCampaignList(props: OutreachPanelProps) {
  ...
  return <div className="space-y-6">{tenant === "lg" && <section className="rounded-lg border border-line p-5"><h2 className="text-xl font-bold">{d.workflow}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{MUNICIPAL_PURPOSES.map(purpose => <Link key={purpose} className={secondary} href={`/admin/zaad?tenant=lg&view=${purpose === "FRAUD_ALERT" ? "one-time" : "campaigns"}&workflow=${purposePath[purpose]}&step=targets`}>{t.municipalOutreach.topics[purposeTopic[purpose]]}</Link>)}</div><Link className="mt-4 inline-flex min-h-11 items-center font-semibold text-primary" href="/admin/zaad?tenant=lg&view=campaigns&step=cases">{d.staffCases}</Link></section>}
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">{d.tabs[2]} <span className="text-base font-normal text-fg-muted">({data.total ?? data.items.length})</span></h2><div className="flex flex-wrap gap-3">{full && permissions.update && <button className={secondary} onClick={() => setBindingsOpen(true)}>{d.bindingTitle}</button>}<button className={secondary} onClick={refresh}>{d.reload}</button><button ref={syncButton} className={secondary} disabled={!full || !permissions.update} title={!full || !permissions.update ? d.campaignSync.permission : undefined} onClick={() => { setSuccess(""); router.push(syncRoute(true), { scroll: false }); }}>{d.sync}</button></div></div>{(!full || !permissions.update) && <p className="text-sm text-fg-muted">{d.campaignSync.permission}</p>}{success && <p role="status">{success}</p>}<p className="text-sm text-fg-muted">{d.liveGate}</p>{!data.items.length ? <p className="py-8 text-center text-fg-muted">{d.campaignSync.empty}</p> : <div className="overflow-x-auto rounded-lg border border-line"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-line bg-surface-hover"><tr>{[d.groupName, z.campaigns.status, d.tabs[1], d.campaignSync.type, z.residents.actions].map(label => <th key={label} scope="col" className="whitespace-nowrap px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-line">{data.items.map(row => <tr key={row.id}><td className="px-4 py-3"><button ref={element => { if (element) rowButtons.current.set(row.id, element); else rowButtons.current.delete(row.id); }} className="cursor-pointer text-left font-semibold hover:text-accent" onClick={() => openCampaign(row.id)}>{row.name}</button></td><td className="whitespace-nowrap px-4 py-3">{d.stateLabels[row.status.toUpperCase()] ?? d.unknown}</td><td className="px-4 py-3">{row.contactListName ?? row.contactListId ?? d.unknown}</td><td className="whitespace-nowrap px-4 py-3">{row.dialingMethod === "agentless" ? "Agentless Dialer" : d.unknown}</td><td className="px-4 py-3"><TableRowActions label={`${row.name}: ${z.residents.actions}`} open={menu === row.id} onOpenChange={open => setMenu(open ? row.id : null)} items={[{ id: "detail", label: z.campaigns.details, onSelect: () => openCampaign(row.id) }, { id: "preview", label: d.preview, disabled: !permissions.update || !["ready", "paused"].includes(row.status.toLowerCase()), disabledReason: d.campaignUi.previewUnavailable, onSelect: () => openCampaign(row.id, true) }]} /></td></tr>)}</tbody></table></div>}
