...
export function OutreachDefaultGroup(props: OutreachPanelProps & { id: string; startEditing?: boolean; close: () => void }) {
  ...
  const [noticeTone, setNoticeTone] = useState<FeedbackTone>("info");
  const errorRef = useRef<HTMLDivElement>(null);
  useEffect(() => { if (!busy && error) errorRef.current?.focus(); }, [error, busy]);
  ...
  async function sync(memberId?: string, resume = false) {
    ...
    try {
      ...
      while (result.status === "PENDING" && mounted.current) {
        ...
        if (mounted.current) { setNoticeTone(syncFeedbackTone(result, true)); setNotice(syncFeedbackMessage(t.outreachCommon.feedback.syncCounts, result.counts)); }
        if (result.counts.pending === pendingBefore) break;
      }
      if (!mounted.current) return;
      setNoticeTone(syncFeedbackTone(result));
      setNotice(syncFeedbackMessage(t.outreachCommon.feedback.syncCounts, result.counts));
      if (result.status !== "PENDING") { sessionStorage.removeItem(storageKey); setOperationId(null); operationKey.current = null; } refresh();
    } catch (failure) { if (mounted.current) { const missing = failure instanceof OutreachApiError && failure.code === "GROUP_NOT_CONFIGURED"; setErrorTone(missing ? "warning" : "error"); setError(missing ? d.missingHelp : common.failure); } }
  ...
  return <section className="space-y-5" aria-labelledby="default-group-title">
    ...
    {!configured && <Feedback tone="warning">{data.group.bindingState === "MISSING" ? d.missingHelp : t.outreachCommon.bindingConflict}</Feedback>}
    {configured && data.providerState !== "READY" && <Feedback tone="error" action={<button className={secondary} onClick={refresh}>{common.retry}</button>}>{d.providerFailure}</Feedback>}

    {notice && <Feedback tone={noticeTone}>{notice}</Feedback>}{error && <Feedback tone={errorTone} ref={errorRef} tabIndex={-1}>{error}</Feedback>}
