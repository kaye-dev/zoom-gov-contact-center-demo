...
export function OutreachDefaultGroup(props: OutreachPanelProps & { id: string; startEditing?: boolean; close: () => void }) {
  ...
  return <section className="space-y-5" aria-labelledby="default-group-title">
    <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2"><h2 id="default-group-title" className="text-lg font-bold">{name}</h2><p className="break-all text-sm text-fg-muted">{d.listId}：{data.group.contactListId ?? d.missing}</p>{writable && <button className="cursor-pointer whitespace-nowrap text-sm text-accent disabled:cursor-not-allowed" disabled={busy} onClick={() => setEditing(true)}>{data.group.contactListId ? d.update : d.configure}</button>}</div><button className={secondary} disabled={busy} onClick={close}>{d.back}</button></div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-4"><div className="flex min-w-0 flex-1 flex-col items-start gap-4 lg:flex-row lg:flex-wrap lg:items-center"><p className="shrink-0 text-lg font-bold">{d.unsynced.replace("{count}", String(unsynced ?? "—"))} <span className="text-sm font-normal text-fg-muted">/ {d.total.replace("{count}", String(total ?? "—"))}</span></p><SearchInput label={common.search} placeholder={common.search} containerClassName="w-full max-w-96 lg:w-96" value={text} onCompositionStart={() => setComposing(true)} onCompositionEnd={event => { setText(event.currentTarget.value); setComposing(false); }} onChange={event => setText(event.target.value)} /></div><div className="flex flex-wrap gap-3">{operationId && writable && <button className={secondary} disabled={busy} onClick={() => void sync(undefined, true)}>{d.resume}</button>}{writable && <button className={primary} disabled={busy || !configured || unsynced === 0} onClick={() => void sync()}>{d.bulk}</button>}</div></div>
    ...
    {!data.items.length ? <p className="py-6">{common.empty}</p> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-line"><tr>{[t.admin.zaad.residents.name, t.admin.zaad.residents.phone, d.source, t.admin.zaad.messages.status, t.admin.zaad.residents.actions].map((label, index) => <th key={label} className={`px-3 py-3 ${index === 4 ? "text-center" : ""}`}>{index === 3 ? <div className="inline-flex items-center gap-2 whitespace-nowrap">{label}<AdminFieldHelp id={`sync-status-help-${id}`} label={d.syncStatusHelp} description={d.boundary} portal /></div> : label}</th>)}</tr></thead><tbody className="divide-y divide-line">{data.items.map(row => {
      ...
      return <tr key={row.id}><td className="px-3 py-4">{row.displayName}</td><td className="whitespace-nowrap px-3 py-4">{row.phones.map(p => p.number).join(" / ")}</td><td className="px-3 py-4">{row.source === "Zoom" ? "Zoom" : d.site}</td><td className="px-3 py-4"><span className={`inline-flex whitespace-nowrap rounded-full bg-surface-hover px-2 py-1 ${row.syncStatus === "FAILED" ? "text-red-700 dark:text-red-300" : "text-fg"}`}>{d.statuses[row.syncStatus]}</span>{row.lastErrorCode === "GROUP_IN_USE" && <p className="mt-1 text-xs text-fg-muted">{d.waiting}</p>}{needsLink && <p className="mt-1 max-w-xs text-xs text-fg-muted">{d.unknownHelp}</p>}{row.retryAfter && <p className="mt-1 text-xs text-fg-muted">{new Date(row.retryAfter).toLocaleString()}</p>}</td><td className="px-3 py-4 text-center"><div className="flex justify-center">{canSync && needsLink ? <TableRowActions label={`${row.displayName}: ${t.admin.zaad.residents.actions}`} open={menu === row.id} onOpenChange={value => setMenu(value ? row.id : null)} items={[{ id: "sync", label: t.outreachCommon.sync, disabled: busy || !configured, onSelect: () => void sync(row.id) }, { id: "link", label: d.reconcile, disabled: busy || !configured, onSelect: () => setLinking(row) }]} /> : canSync ? <button className="inline-flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-md text-fg-muted hover:text-accent hover:bg-surface-hover focus-visible:outline-2 focus-visible:outline-accent disabled:opacity-50 disabled:cursor-not-allowed" aria-label={t.outreachCommon.sync} title={t.outreachCommon.sync} disabled={busy || !configured || row.syncStatus === "SYNCING"} onClick={() => void sync(row.id)}><RefreshIcon /></button> : "—"}</div></td></tr>;
    })}</tbody></table></div>}
