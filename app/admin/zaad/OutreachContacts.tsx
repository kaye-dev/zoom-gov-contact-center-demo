...
export function OutreachContacts(props: OutreachPanelProps) {
  ...
  useEffect(() => {
    if (contactState !== "registration-settings") return;
    const params = new URLSearchParams(queryString);
    ["state", "detail", "importJob", "query", "cursor", "page", "origin", "trail"].forEach(key => params.delete(key));
    router.replace(`/admin/zaad?${params}`, { scroll: false });
  }, [contactState, queryString, router]);
  ...
  return <div className="space-y-5"><div data-contact-list-toolbar className="flex min-w-0 flex-col items-start gap-4 lg:flex-row lg:items-center lg:gap-3"><h1 className="shrink-0 text-2xl font-bold">{d.defaultGroups.manageContacts} <span className="text-sm font-normal text-fg-muted">({loading ? "—" : data?.total ?? "—"})</span></h1><SearchInput label={z.common.search} placeholder={z.common.search} autoComplete="off" containerClassName="w-full min-w-0 lg:flex-1" value={text} onCompositionStart={() => setComposing(true)} onCompositionEnd={event => { setText(event.currentTarget.value); setComposing(false); }} onChange={event => setText(event.target.value)} /><div data-contact-list-actions className="flex w-full flex-nowrap gap-1.5 [&>button]:min-h-11 [&>button]:whitespace-nowrap [&>button]:px-2 [&>button]:text-xs sm:w-auto sm:gap-2 sm:[&>button]:px-3 sm:[&>button]:text-sm lg:ml-auto lg:shrink-0"><button className={secondary} disabled={busy} onClick={refresh}>{d.reload}</button>{permissions.create && <><button className={secondary} disabled={busy} onClick={() => importPage(true)}>{d.csvRegister}</button><button className={primary} disabled={busy} onClick={() => { returnContactKey.current = null; router.push(contactHref("contact-add"), { scroll: false }); }}>{tenant === "lg" ? d.residentRegister : d.studentRegister}</button></>}</div></div><DetailPageBreadcrumb title={d.defaultGroups.manageContacts} disabled={busy} />
    {failed ? <OutreachFailure retry={refresh} /> : <div className={outreachTableFrame}><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-surface-hover"><tr>{[z.residents.name, z.residents.phone, ...(tenant === "univ" ? [t.universityOutreach.studentNumber] : []), d.topic, d.registrationStatus, d.source, z.residents.actions].map(label => <th key={label} className={`whitespace-nowrap px-4 py-3 font-semibold ${label === z.residents.actions ? "text-center" : ""}`}>{label}</th>)}</tr></thead><tbody className="divide-y divide-line [&>tr:first-child]:border-t [&>tr:first-child]:border-line">{loading ? <OutreachTableSkeleton columns={tenant === "lg" ? 6 : 7} /> : (data?.items ?? []).map(row => <tr key={referenceKey(row.reference)} className={busy ? "" : "cursor-pointer hover:bg-surface-hover/40"} onClick={event => handleOutreachRowClick(event, () => openContact(row, "detail"), busy)}><td className="px-4 py-3 font-medium"><button className="cursor-pointer text-left text-accent hover:underline" data-contact-key={referenceKey(row.reference)} onClick={() => openContact(row, "detail")}>{row.name}</button></td><td className="whitespace-nowrap px-4 py-3">{row.phone}</td>{tenant === "univ" && <td className="px-4 py-3">{row.studentNumber}</td>}<td className="px-4 py-3">{row.requestedTopics.map(topic => tenant === "lg" ? t.municipalOutreach.topics[topic as keyof typeof t.municipalOutreach.topics] : t.universityOutreach.topicLabels[["scholarship", "class-change", "facility", "group", "continuity"].indexOf(topic)]).filter(Boolean).join(" / ") || "—"}</td><td className="whitespace-nowrap px-4 py-3">{status(row.status)}</td><td className="px-4 py-3">{row.source === "MANUAL" ? d.manual : row.source === "UNKNOWN" ? d.unknown : row.source}</td><td className="px-4 py-3"><div className="flex justify-center"><TableRowActions label={`${row.name}: ${z.residents.actions}`} open={menu === referenceKey(row.reference)} onOpenChange={value => setMenu(value ? referenceKey(row.reference) : null)} items={[{ id: "edit", label: permissions.update ? z.common.edit : d.select, onSelect: () => openContact(row, permissions.update ? "edit" : "detail") }, ...(permissions.delete && row.reference.origin !== "DISASTER_RADIO" ? [{ id: "delete", label: z.common.delete, tone: "danger" as const, onSelect: () => { setDeleting(row); setReason(""); setError(""); } }] : [])]} /></div></td></tr>)}</tbody></table></div>}
    ...
    {deleting && <ModalDialog initialFocusRef={deleteCancelRef} title={d.deleteContact} description={deleting.name} locked={busy} onRequestClose={() => { if (!lock.current) setDeleting(null); }}><form className="space-y-4" onSubmit={event => { event.preventDefault(); void remove(); }}><p className="text-sm text-fg-muted">{d.deleteContactHelp}</p>{error && <Feedback tone="error">{error}</Feedback>}<label className="block">{d.deleteReason}<textarea required maxLength={1000} className={input} disabled={busy} value={reason} onChange={event => setReason(event.target.value)} /></label><div className="flex justify-end gap-3"><button ref={deleteCancelRef} type="button" className={secondary} disabled={busy} onClick={() => setDeleting(null)}>{z.common.cancel}</button><button className={primary} disabled={busy}>{z.common.delete}</button></div></form></ModalDialog>}
