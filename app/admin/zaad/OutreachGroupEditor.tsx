...
export type OutreachGroup = { kind?: "DEFAULT" | "REGULAR"; topicKey?: string; contactListId?: string | null; bindingState?: string; id: string; name: string; description: string; version: number; revision: string; updatedAt?: string | null; contactCount: number | null; mutationBlock?: "GROUP_IN_USE" | "CAMPAIGN_REFERENCE_UNKNOWN" | null };
export function OutreachGroupEditor({ group, close, saved, tenant, permissions, setDirty, setSaving }: OutreachPanelProps & { group: OutreachGroup | null; close: () => void; saved: () => void }) {
  const { t } = useI18n(), z = t.admin.zaad, d = t.outreachCommon;
  const [draft, setDraft] = useState(() => group ?? { id: "", name: "", description: "", version: 0, revision: "", contactCount: 0 });
  ...
  async function save() {
    ...
    try {
      await outreachMutation(tenant, draft.id ? `contact-lists/${encodeURIComponent(draft.id)}` : "contact-lists", { operationKey: operation.current, name: draft.name, description: draft.description, ...(draft.id ? { version: draft.version, expectedRevision: draft.revision } : {}) }, draft.id ? "PATCH" : "POST");
