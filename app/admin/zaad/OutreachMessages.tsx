...
export function OutreachMessages(props: OutreachPanelProps) {
  ...
  if (editing && (loadedKey !== routeKey || detailFailure === routeKey)) return <div className="space-y-4"><button className={secondary} onClick={back}>{d.backToContacts}</button>{detailFailure === routeKey ? <OutreachFailure retry={() => { setDetailFailure(""); refresh(); }} /> : <OutreachLoading />}</div>;
