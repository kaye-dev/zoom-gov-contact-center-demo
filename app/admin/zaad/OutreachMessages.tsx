...
import { OutreachImportedAudioDetail } from "./OutreachImportedAudioDetail";
...
export function OutreachMessages(props: OutreachPanelProps) {
  ...
  const { t } = useI18n(), z = t.admin.zaad, d = t.outreachCommon, m = d.messageUi, c=t.outreachCommon.messageImport;
  ...
  const [importNotice,setImportNotice]=useState(""); const importNoticeRef=useRef<HTMLParagraphElement>(null);
  useEffect(()=>{if(importNotice && state !== "message-sync")importNoticeRef.current?.focus();},[importNotice,state]);
  const [menu, setMenu] = useState<string | null>(null);
  useEffect(() => { const controller = new AbortController(); outreachRequest<ListResult<CatalogMessage>>(tenant, "message-catalog", { signal: controller.signal }).then(result => { if (!controller.signal.aborted) setData(result); }).catch(() => { if (!controller.signal.aborted) setFailed(true); }); return () => controller.abort(); }, [tenant, reload]);
  useEffect(() => {
  ...
  function edit(row?: CatalogMessage) { if(row&&isImportedAudio(row)){router.push(href("message-audio-detail",row.id),{scroll:false});return;} setSaved(false); router.push(href(row ? "message-edit" : "message-create", row?.id), { scroll: false }); }
  ...
  function actions(row: CatalogMessage): TableRowAction[] {
    if(isImportedAudio(row)) return [{id:"detail",label:c.detail,disabled:busy,onSelect:()=>edit(row)}];
...
function ImportedAudioLoader({ tenant, id }: { tenant: OutreachPanelProps["tenant"]; id: string }) {
  ...
  return message ? <OutreachImportedAudioDetail message={message} /> : <section className="space-y-5"><h1 className="text-2xl font-bold">{title}</h1><DetailPageBreadcrumb title={title}/>{failed ? <OutreachFailure retry={() => { setFailed(false); setReload(value => value+1); }}/> : <OutreachLoading/>}</section>;
}
