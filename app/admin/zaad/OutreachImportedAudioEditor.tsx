...
export function OutreachImportedAudioEditor({message,tenant,permissions,fullAccess,setDirty,setSaving,saved}:OutreachPanelProps&{message:ImportedAudioMessage;saved:()=>void}){
 ...
 async function save(){
   if(lock.current||!writable||!confirming)return;lock.current=true;setBusy(true);setSaving?.(true);setError("");
   try{
     ...
     setDirty(false);setConfirming(false);saved();
   }catch(cause){
