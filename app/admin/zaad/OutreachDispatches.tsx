...
import type { DispatchConfirmationStatus, DispatchHistory, DispatchRecord } from "@/app/admin/zaad/outreach-dispatch-types";
...
export function OutreachDispatches(props: OutreachPanelProps) {
  ...
  if (routed && (loadedKey !== routeKey || detailFailure === routeKey)) return <div className="space-y-4"><h1 className="text-2xl font-bold">{d.snapshot}</h1><DetailPageBreadcrumb title={d.snapshot} />{detailFailure === routeKey ? <OutreachFailure retry={() => { setDetailFailure(""); refresh(); }} /> : <OutreachLoading />}</div>;
  ...
  return <div><OutreachListActions>{permissions.create && <button className={outreachTabAction} onClick={() => go("dispatch-create")}>{z.common.create}</button>}</OutreachListActions>

    {failed && <OutreachFailure inline retry={refresh} />}{tenant === "lg" ? <OutreachPurposeBindings {...props} loading={!data && !failed} mode="one-time">{!data && !failed ? <OutreachTableSkeleton columns={5} /> : (data?.items ?? []).map(row => <tr key={row.id} className="cursor-pointer hover:bg-surface-hover/40" onClick={event => handleOutreachRowClick(event, () => go("dispatch-history", row.id), false)}><td className="px-4 py-3"><button className="cursor-pointer text-left font-semibold hover:text-accent" onClick={() => go("dispatch-history", row.id)}>{row.name}</button></td><td className="whitespace-nowrap px-4 py-3">{new Date(row.createdAt).toLocaleString(locale)}</td><td className="whitespace-nowrap px-4 py-3">{row.recipientCount}</td><td className="whitespace-nowrap px-4 py-3">{d.stateLabels[row.appState] ?? d.unknown}</td><td className="px-4 py-3"><TableRowActions label={`${row.name}: ${z.residents.actions}`} open={menu === row.id} onOpenChange={value => setMenu(value ? row.id : null)} items={[{ id: "detail", label: d.snapshot, onSelect: () => go("dispatch-history", row.id) }, ...(permissions.create && canRepeat(row) ? [{ id: "rerun", label: d.rerun, onSelect: () => go("dispatch-retry", row.id) }] : [])]} /></td></tr>)}</OutreachPurposeBindings> : data && !data.items.length ? <p className="rounded-lg border border-line p-8 text-center">{z.common.empty}</p> : <div className={outreachTableFrame}><table aria-busy={!data && !failed} className="w-full min-w-[720px] text-left text-sm"><thead className="bg-surface-hover"><tr>{[d.groupName, u.createdAt, z.oneTime.uniqueRecipients, u.status, z.residents.actions].map(label => <th key={label} className="whitespace-nowrap px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-line [&>tr:first-child]:border-t [&>tr:first-child]:border-line">{!data && !failed ? <OutreachTableSkeleton columns={5} /> : (data?.items ?? []).map(row => <tr key={row.id} className="cursor-pointer hover:bg-surface-hover/40" onClick={event => handleOutreachRowClick(event, () => go("dispatch-history", row.id), false)}><td className="px-4 py-3"><button className="cursor-pointer text-left font-semibold hover:text-accent" onClick={() => go("dispatch-history", row.id)}>{row.name}</button></td><td className="whitespace-nowrap px-4 py-3">{new Date(row.createdAt).toLocaleString(locale)}</td><td className="whitespace-nowrap px-4 py-3">{row.recipientCount}</td><td className="whitespace-nowrap px-4 py-3">{d.stateLabels[row.appState] ?? d.unknown}</td><td className="px-4 py-3"><TableRowActions label={`${row.name}: ${z.residents.actions}`} open={menu === row.id} onOpenChange={value => setMenu(value ? row.id : null)} items={[{ id: "detail", label: d.snapshot, onSelect: () => go("dispatch-history", row.id) }, ...(permissions.create && canRepeat(row) ? [{ id: "rerun", label: d.rerun, onSelect: () => go("dispatch-retry", row.id) }] : [])]} /></td></tr>)}</tbody></table></div>}
  </div>;
}

function OutreachDispatchSnapshot({ record, confirmation, confirmationStatus, retry, writable, edit, rerun }: { record: DispatchRecord; confirmation: boolean; confirmationStatus: DispatchConfirmationStatus; retry: () => void; writable: boolean; close: () => void; edit: () => void; rerun: () => void }) {
  ...
  return <section className="max-w-4xl space-y-6" aria-labelledby="dispatch-snapshot-title"><div className="flex flex-wrap items-center justify-between gap-3"><h1 ref={title} tabIndex={-1} id="dispatch-snapshot-title" className="text-2xl font-bold">{confirmation ? u.confirmTitle : record.name}</h1></div><DetailPageBreadcrumb title={confirmation ? u.confirmTitle : record.name} disabled={false} />
    <p className="text-sm text-fg-muted">{d.stateLabels[record.appState] ?? d.unknown}</p>
    {confirmation && confirmationStatus !== "CURRENT" && <Feedback tone={confirmationStatus === "ALREADY_REQUESTED" ? "info" : confirmationStatus === "UNAVAILABLE" ? "error" : "warning"} action={confirmationStatus === "UNAVAILABLE" ? <button className={secondary} onClick={retry}>{z.common.retry}</button> : undefined}>{u.confirmationStatuses[confirmationStatus]}</Feedback>}
    {!snapshot ? <Feedback tone="warning">{z.oneTime.snapshotPending}</Feedback> : <><dl className="grid gap-3 text-sm"><dt className="font-semibold">{d.groupName}</dt><dd>{snapshot.name}</dd><dt className="font-semibold">{u.connection}</dt><dd>{snapshot.connectionMode === "FLOW" ? d.flow : d.media}</dd><dt className="font-semibold">{z.messages.body}</dt><dd className="whitespace-pre-wrap break-words leading-7">{snapshot.body}</dd><dt className="font-semibold">{z.messages.voice}</dt><dd>{snapshot.voiceId} · {snapshot.languageCode}</dd><dt className="font-semibold">{z.oneTime.uniqueRecipients}</dt><dd className="text-2xl font-bold">{snapshot.recipientCount}</dd></dl>
      ...
      {snapshot.recipientCount === 0 && <Feedback tone="warning">{d.noEligibleTargets}</Feedback>}
