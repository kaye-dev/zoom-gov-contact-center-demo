...
import { AdminSectionNavigation } from "@/app/admin/AdminSectionNavigation";
import { ConfirmationDialog } from "@/app/admin/users/ConfirmationDialog";
...
export type RolesActions = {
  createRole: (input: { name: string; description: string }) => Promise<{
    ok: boolean;
    role?: { id: string };
    error?: string;
  }>;
  deleteRole: (id: string, expectedRevision: number) => Promise<{ ok: boolean; status: number }>;
  onCreated: (id: string) => void;
  onDeleted: (href?: string) => void;
};

export function RolesPanel({
  roles, total, page, totalPages, pageSize, search,
  canCreate, canUpdate, canDelete, canViewMembers, actions,
}: RolesData & { actions: RolesActions }) {
  const { t } = useI18n();
  ...
  const removeRole = async () => {
    ...
    setDeleteError(null);
    try {
      const response = await actions.deleteRole(roleToDelete.id, roleToDelete.revision);
      if (!response.ok) {
        setDeleteError(
      ...
      actions.onDeleted(roles.length === 1 && page > 1
        ? buildRoleListHref(search, page - 1, pageSize)
        : undefined);
    } catch {
  ...
  return (
    <section data-row-action-region className="min-w-0">
      ...
      {isCreateOpen ? (
        <CreateRoleDialog actions={actions} onClose={() => setIsCreateOpen(false)} />
      ) : null}
...
function CreateRoleDialog({ onClose, actions }: { onClose: () => void; actions: RolesActions }) {
  const { t } = useI18n();
  const copy = t.admin.accessControl;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setNameError(null);
    try {
      const body = await actions.createRole({ name, description });
      if (!body.ok || !body.role?.id) {
        if (
      ...
      actions.onCreated(body.role.id);
    } catch {
