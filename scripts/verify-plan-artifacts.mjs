...
export function invalidPlanArtifacts(trackedPaths) {
  return trackedPaths.filter((trackedPath) => (
    trackedPath.startsWith("plans/")
    || (trackedPath.startsWith("plan/") && trackedPath !== "plan/template.md")
  ));
}
...
export function verifyPlanArtifacts(trackedPaths) {
  ...
  throw new Error(`Only plan/template.md may be tracked. Delete these plan artifacts:\n${invalid.map((item) => `- ${item}`).join("\n")}`);
}
...
if (import.meta.filename === process.argv[1]) {
  try {
    ...
    console.log("Plan artifact guard passed: only plan/template.md is tracked.");
  } catch (error) {
