...
export const TEMPLATE_BODY_SENTINELS = [
  ...
  "- comparison conditions: 実画面とprototypeがそれぞれ報告したviewport、devicePixelRatio、scrollX・scrollY、locale、document theme class、fixture、query state。指定値でなく両画面の実測値を記録する",
  ...
  "- theme contract: productionが対応する全theme、default、document class、初期同期、保存、既存toggle配置、reviewer entry point、各stateのlight・dark検証を記録する",
  "- responsive contract: affected sourceのlayout-changing breakpoint、390×844、desktop、各breakpointの1px手前・境界、state別transform、横overflow結果を記録する",
  "- styling pipeline: 本番と同じTailwind CSS utility、app/globals.css、theme bootstrapを使用したこと。手書きCSSがある場合は事前のユーザー承認記録と例外理由",
  ...
  "- parity evidence: 条件を揃えた同一stateのlight・dark screenshot pair、unchanged regionのbounding rect・computed style・DOM・a11y比較、breakpoint境界、theme切替、overflow、console、network、未解消差分",
  "- production UI正本: 承認済みprototypeを実装後の完成UI契約とし、実装時に未承認の構造・文言・component・responsive・interaction変更を行わないこと",
  "- parity matrix: 影響するroute・overlay・baseline state・新規stateごとのentry point、light・dark、両画面の実測条件、screenshot、unchanged region、意図した差分ID、desktop、390×844、関連breakpoint境界、keyboard・focus・theme切替、未決事項",
