...
import { outreachWhere, type OutreachScope } from "./outreach-scope";
...
export async function listContacts(db: Database, scope: OutreachScope, query: { search?: string; cursor?: string; limit?: number } = {}) {
  ...
  if (scope.siteKey === "lg") {
    ...
    items.push(...rows.map(row => ({ reference: reference(scope, "MUNICIPAL_CONTACT", row.id), name: row.name, phone: row.phone, studentNumber: null, source: contactSource(row.source), status: row.status, topics: row.preferences.filter(p => p.enabled).map(p => p.topic), requestedTopics: row.preferences.filter(p => p.requested).map(p => p.topic), identityVerified: row.identityVerified, phoneVerified: row.phoneVerified, version: row.version, district: row.district })));
    {
    ...
    items.push(...rows.map(row => ({ reference: reference(scope, "UNIVERSITY_CONTACT", row.id), name: row.name, phone: row.phone, studentNumber: row.displayStudentNumber, source: contactSource(row.registrationSource), status: row.deletedAt ? "WITHDRAWN" : row.phoneEligible && row.identityVerified && row.phoneVerified ? "ACTIVE" : row.registrationStatus, topics: row.preferences.filter(p => p.enabled).map(p => p.topicId), requestedTopics: row.preferences.filter(p => p.enabled).map(p => p.topicId), identityVerified: row.identityVerified, phoneVerified: row.phoneVerified, version: row.version })));
    {
      ...
      items.push(...registrations.map(row => ({ reference: reference(scope, "UNIVERSITY_REGISTRATION", row.id), name: row.reviewedName ?? row.name, phone: row.reviewedPhone ?? row.phone, studentNumber: studentNumber(row.facultyCode, row.admissionYear, row.serial), source: contactSource(row.source), status: row.status, topics: [], requestedTopics: row.reviewedAt ? row.reviewedTopicIds : row.topicIds, identityVerified: row.identityConfirmed, phoneVerified: row.phoneConfirmed, version: row.version, consentVersion: row.consentVersion })));
    }
  }
  if (scope.siteKey === "univ") {
    ...
    items.push(...candidates.map(row => ({ reference: reference(scope, "IMPORT_CANDIDATE", row.id), name: row.name, phone: row.phone, studentNumber: row.studentNumber, source: contactSource(row.source), status: row.status, topics: [], requestedTopics: [], identityVerified: false, phoneVerified: false, version: row.version })));
  }
...
export async function getContact(db: Database, scope: OutreachScope, origin: PersonOrigin, id: string): Promise<ContactDto & Record<string, unknown>> {
  if (origin === "IMPORT_CANDIDATE" && scope.siteKey === "univ") {
    ...
    return { reference: reference(scope, origin, id), name: row.name, phone: row.phone, studentNumber: row.studentNumber, source: contactSource(row.source), status: row.status, topics: [], requestedTopics: [], identityVerified: false, phoneVerified: false, version: row.version, linkedContactId: row.linkedContactId };
  }
  if (origin === "MUNICIPAL_CONTACT" && scope.siteKey === "lg") {
    ...
    if (!row) throw new OutreachContractError("NOT_FOUND", 404);
    return { reference: reference(scope, origin, id), name: row.name, phone: row.phone, studentNumber: null, source: contactSource(row.source), status: row.status, version: row.version, identityVerified: row.identityVerified, phoneVerified: row.phoneVerified, topics: row.preferences.filter(p => p.enabled).map(p => p.topic), requestedTopics: row.preferences.filter(p => p.requested).map(p => p.topic), preferences: row.preferences, availability: row.availability[0] ?? null, district: row.district, confirmationMethod: row.confirmationMethod, confirmedAt: row.confirmedAt, attestation: row.attestation };
  }
  if (origin === "UNIVERSITY_REGISTRATION" && scope.siteKey === "univ") {
    const row = await getRegistration(db, scope as Scope, id);
    return { reference: reference(scope, origin, id), name: row.name, phone: row.phone, studentNumber: row.displayStudentNumber, source: contactSource(row.source), status: row.status, version: row.version, identityVerified: row.identityConfirmed, phoneVerified: row.phoneConfirmed, topics: row.status === "ACTIVE" ? row.topicIds : [], requestedTopics: row.topicIds, registration: row };
  }
  ...
  try {
    await db.$transaction(async tx => {
      ...
      if (scope.siteKey === "lg") {
        ...
        const changed = await tx.municipalContact.updateMany({ where: { ...outreachWhere(scope), id, version }, data: { name, phone, status: nextStatus, district: choice(v.district, MUNICIPAL_DISTRICTS), identityVerified: !phoneChanged && v.identityVerified === true, phoneVerified: !phoneChanged && v.phoneVerified === true, confirmedBy: scope.actorId, confirmedAt, confirmationMethod, attestation, version: { increment: 1 } } });
        ...
        if (phoneChanged || nextStatus !== "ACTIVE" || current.topics.some(topic => !topics.includes(topic))) {
        ...
        const changed = await tx.universityContact.updateMany({ where: { ...outreachWhere(scope), id, version }, data: { name, phone, registrationStatus: nextStatus, identityVerified: !phoneChanged && v.identityVerified === true, phoneVerified: !phoneChanged && v.phoneVerified === true, phoneEligible: nextStatus === "ACTIVE", ...(number ? { facultyCode: number[0], admissionYear: 2000 + Number(number.slice(1, 3)), serial: number.slice(3), displayStudentNumber: number } : {}), version: { increment: 1 } } });
        if (changed.count !== 1) throw new OutreachContractError("VERSION_CONFLICT", 409);
...
async function completeImportCandidate(db: PrismaClient, scope: OutreachScope, id: string, payload: unknown) {
  ...
  const version = whole(v.version), name = stringValue(v.name), phone = phoneValue(v.phone), number = stringValue(v.studentNumber, 7);
