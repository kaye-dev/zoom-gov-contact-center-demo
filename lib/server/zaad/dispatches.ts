...
export async function dispatchTargets(db: PrismaClient, scope: OutreachScope, draft: ReturnType<typeof parseDispatchDraft>, injected?: GroupClient) {
  ...
  for (const selected of refs) {
    ...
    targets.push({ ...selected, name: contact.name, personKey: referenceKey(selected.reference), phone: contact.phone, contactVersion: contact.version, consentDigest: digest(contact.preferences ?? { topics: contact.topics, consentVersion: contact.consentVersion }) });
  }
...
export async function preflightDispatch(db: PrismaClient, scope: OutreachScope, payload: unknown, injected?: GroupClient) {
  const draft = parseDispatchDraft(payload, scope.siteKey);
  ...
  if (draft.messageRevisionId) {
    messageRevision = await db.messageRevision.findFirst({ where: { siteKey: scope.siteKey, id: draft.messageRevisionId, message: { retiredAt: null } } });
  ...
  if (draft.flowBindingId) {
    const binding = await db.zoomResourceBinding.findFirst({ where: { id: draft.flowBindingId, ownerSiteKey: scope.siteKey, resourceType: "FLOW", tombstone: false } });
  ...
  const snapshot = { scopeContract: "tenant-v1", ...selection, name: draft.name, body: draft.body, voiceId: draft.voiceId, languageCode: draft.languageCode, connectionMode: draft.connectionMode, messageRevisionId: messageRevision?.id ?? null, contentDigest: digest({ body: draft.body, voiceId: draft.voiceId, languageCode: draft.languageCode }), flow, actorId: scope.actorId, draftDigest: digest(draft), createdAt: new Date().toISOString() };
  try {
...
export async function getDispatch(db: PrismaClient, scope: OutreachScope, id: string) {
  const row = await db.zaadOneTimeDispatch.findFirst({ where: { ...outreachWhere(scope), id }, include: { messageRevision: { select: { messageId: true, name: true, id: true } } } });
...
export async function listDispatches(db: PrismaClient, scope: OutreachScope) {
  const items = await db.zaadOneTimeDispatch.findMany({ where: outreachWhere(scope), orderBy: [{ createdAt: "desc" }, { id: "asc" }], select: { id: true, name: true, siteKey: true, appState: true, state: true, recipientCount: true, createdAt: true, revision: true, parentDispatchId: true } });
...
export async function inspectDispatchConfirmation(db: PrismaClient, scope: OutreachScope, id: string, injected?: GroupClient) {
  ...
  if (dispatch.appState !== "VALIDATING") confirmationStatus = "ALREADY_REQUESTED";
    ...
    try { await assertCurrentSelection(db, scope, dispatch, injected); }
      ...
      confirmationStatus = error instanceof OutreachContractError && ["TARGET_CHANGED", "NOT_FOUND"].includes(error.code) ? "TARGET_CHANGED" : "UNAVAILABLE";
    }
...
async function assertCurrentSelection(db: PrismaClient, scope: OutreachScope, dispatch: Awaited<ReturnType<typeof getDispatch>>["dispatch"], injected?: GroupClient) {
  ...
  if (!saved || typeof saved !== "object" || Array.isArray(saved) || saved.scopeContract !== "tenant-v1") throw new OutreachContractError("TARGET_CHANGED", 409);
